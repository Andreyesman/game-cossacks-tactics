...
func _handle_get_request(peer: StreamPeerTCP, parsed: Dictionary) -> void:
	...
	# OAuth discovery endpoints — return JSON 404 so clients skip OAuth entirely
	if parsed["path"].begins_with("/.well-known/"):
		var json_body: String = "{}"
		var json_bytes: PackedByteArray = json_body.to_utf8_buffer()
		var resp: String = "HTTP/1.1 404 Not Found\r\n"
		resp += "Content-Type: application/json; charset=utf-8\r\n"
		resp += "Content-Length: " + str(json_bytes.size()) + "\r\n"
		resp += "Access-Control-Allow-Origin: *\r\n"
		resp += "\r\n"
		peer.put_data(resp.to_utf8_buffer() + json_bytes)
		peer.disconnect_from_host()
		return
