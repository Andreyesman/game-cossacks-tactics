...
**✅ Сесія 12.06.2026 (v4.9) — UI polish (4.5.6): UIStyle.gd, дерев'яні кнопки, орнаментні рамки, курсор-перо, градієнт фону меню — див. секцію 4.5.6**
**✅ Сесія 12.06.2026 (v4.8) — закрито 4.5.5: процедурна географія глобальної карти (доробка: засідки в болотах/ярах, видалення мертвих прямокутних біомів), відновлено пошкоджені секції 4.8/4.9 плану — див. секцію 4.5.5**
...
- [x] **Створити `assets/audio/music/map_theme.wav`** — ✅ додано, імпортовано Godot
- [x] **Створити `assets/audio/music/battle_theme.wav`** — ✅ додано, імпортовано Godot
- [x] **Музика зациклена** — ✅ виправлено (05.06.2026)
- [x] **WAV → OGG** — ✅ v4.7 (12.06.2026): battle_theme 36→2 МБ, map_theme 29→1.5 МБ (q:a 6), див. 0.1.5

> **Реалізовано (v3.3):** `AudioManager.gd` повністю реалізовано. `TRACKS` dict вказує на `map_theme.wav` / `battle_theme.wav`. Якщо файл відсутній — `push_warning`, гра не падає. Call sites підключені у всіх трьох сценах.
>
> **Реалізовано (v3.4):** `map_theme.wav` та `battle_theme.wav` додані у `assets/audio/music/`. Обидва файли імпортовані Godot (`.wav.import`). Музика в грі тепер грає.
>
> **Bugfix (05.06.2026):** Музика грала один раз і зупинялась. Спроба встановити `AudioStreamWAV.loop_mode` + `loop_end = 0` в коді повністю ламала відтворення (`loop_end=0` означає "кінець петлі на семпл 0"). Рішення: loop через сигнал `finished`. В `_create_players()` кожен музичний плеєр підключається до `_on_music_player_finished(player)`. Callback перезапускає плеєр лише якщо він є активним (`player == _music_players[_active_music_idx]`). Попередній плеєр після кросфейду зупиняється через `stop()`, який не емітує `finished` — зайвих рестартів немає.

**Оцінка:** ✅ ЗАВЕРШЕНО — Код + Файли + Loop + Robustness.

> **Bugfix #2 (05.06.2026):** Після проміжного стану broken AudioManager.gd (метод `_on_music_player_finished` був referenced but not declared між двома правками) — Godot кешував помилку скрипта, Autoload не ініціалізувався. `AudioManager.play_music("battle")` в `BattleManager._ready()` падав на рядку 43, вся ініціалізація зупинялась — бежевий фон без юнітів. Фікс: виклик через `get_node_or_null("/root/AudioManager")` + null check + `push_warning`. Тепер бій запускається навіть без AudioManager. Додано `print("BattleManager: _ready() ЗАВЕРШЕНО")` для діагностики.

---

### 4.9 Аудіо — SFX ✅ ЗАВЕРШЕНО (v3.6)

**Що:** 9 звукових ефектів для ключових дій.

- [x] `AudioManager.play_sfx(key)` — SFX-пул 5 плеєрів, round-robin
- [x] `SFX` dict: `hit_melee`, `hit_ranged`, `death`, `ui_click`, `buy`, `level_up`, `heal`, `hire`, `notification`
- [x] **Підключити `AudioManager.play_sfx()`** до подій у коді:
  - `CombatUnit._perform_attack_logic` → `"hit_melee"` (при `is_hit == true`)
  - `CombatUnit.execute_shoot` → `"hit_ranged"` (перед await анімації польоту)
...
### 4.5.5 Глобальна карта — процедурна географія та зони впливу ✅ ЗАВЕРШЕНО (v4.8)
...
> **Реалізовано (більшість — у коміті «Map generator», доведено до кінця у v4.8 — 12.06.2026):**
> - **`WorldGenerator.gd`:** `_gen_geography()` — ліси (blob-полігони, густіші на заході), пагорби (витягнуті хребти під кутом), болота (biased до рік/озер), кургани (4–7), яри (меандри 10+ сегментів). `_gen_influence_zones()` — `influence_anchors` по містах. Прямокутні `biomes` повністю видалено (`_gen_biomes()` — мертвий код, прибрано у v4.8; ключ `biomes` зник із `world_state`).
> - **`WorldMap.gd`:** `_render_biomes()` — пергаментний шейдер (fbm-шум, плями старіння, віньєтка) + `_render_geography_patches()` + `_render_cartographic_symbols()` (дерева, штрихування пагорбів, хвилі боліт у стилі карт XVII ст.). Кольорових смуг фракцій немає. `_get_biome_at(pos)` — point-in-polygon по патчах. `get_terrain_speed_modifier()`: ліс 0.70, болото 0.50, пагорб 0.60, яр 0.40. `_get_biome_modifier()` (радіус туману): ліс 0.7, пагорб 1.2 (на зупинці), курган 1.5. `get_faction_at(pos)` — Voronoi-lookup по `influence_anchors`.
> - **Засідки (v4.8):** `WorldMap.get_ambush_detection_multiplier(pos)` → ×1.6 у болоті/яру; `EnemyParty._ambush_multiplier()` — лише бандитські загони помічають гравця далі; посилання на WorldMap кешовано (`_world_map_ref`), без пошуку щокадру.
> - Спавн ворожих партій прив'язаний до трьох X-секторів (Захід/Центр/Схід) — варіант «за координатою X» зі специфікації.
> - Верифікація (headless): smoke-тест генератора — 25 локацій, 21 ліс, 9 пагорбів, 8 боліт, 5 курганів, 3 яри, 5 якорів, ключа `biomes` немає; компіляція WorldMap/EnemyParty/CampaignManager чиста.
> - **Залишок (арт):** заміна процедурних символів на фінальні текстури — беклог «Текстури глобальної карти».

---

### 4.5.6 UI polish ✅ ЗАВЕРШЕНО (v4.9 — 12.06.2026)

**Що:** Загальна стилізація UI під козацьку тематику.

- [x] **TopBar глобальної карти — Figma-дизайн** (3 пергаментні панелі) — ✅ v4.2
- [x] Фон головного меню — увімкнено градієнтне затемнення зліва (`_add_gradient_overlay()` існувала, але не викликалась)
- [x] Стилізовані кнопки — дерев'яна текстура (StyleBoxTexture 9-patch + tint для семантичних кольорів)
- [x] Рамки панелей — орнаментна рамка (золоті завитки в кутах, подвійна лінія по краях)
- [x] Курсор — гусяче перо (`Globals._setup_custom_cursor()`, hotspot у кінчику пера)

> **Реалізовано (v4.9):**
> - **`src/ui/UIStyle.gd`** (новий, `class_name`, не autoload): `apply_button(btn, tint, margins)` / `apply_panel(panel, tint, margins, fallback_border)` / `tint_from(base)`. StyleBoxTexture з 9-patch; фолбек на StyleBoxFlat у палітрі гри, якщо SVG відсутній (патерн TERRAIN_TEXTURES/ITEM_ICONS — гра не падає без файлів).
> - **5 SVG-асетів** у `assets/sprites/ui/` (тимчасові, процедурні — Figma-арт замінить файли за тими ж шляхами): `btn_wood_normal/hover/pressed.svg` (192×64, 9-patch margin 16), `panel_frame_ornate.svg` (192×192, margin 56 — орнамент у кутах сягає ~53px), `cursor_quill.svg` (48×48).
> - **Кнопки:** Settings «Закрити» (MainMenu), `BattleResultScreen._create_button()`, `WorldMap._make_dialog_btn()/_make_trade_btn()`, `BattleManager._style_end_turn_button()` (зелений tint), кнопки `EncounterDialog` (червоний/нейтральний/зелений tint — code-override, `.tscn` не чіпали).
> - **Рамки панелей:** Settings, CharacterSheet, BattleResultScreen (червонуватий tint при поразці), Game Over, діалог поселення (WorldMap), EncounterDialog.
> - **Свідомо не чіпали:** великі кнопки MainMenu (Tween анімує `StyleBoxFlat.bg_color` — несумісно зі StyleBoxTexture, і це навмисний дизайн), TopBar (Figma), нутрощі UnitPanel/CombatLog/TurnQueueUI, `game_theme.tres`.
> - Верифікація: headless-компіляція 7 скриптів чиста; візуально в грі — градієнт меню, Settings (рамка + дерев'яна «Закрити»), EncounterDialog (рамка + 2 tint-кнопки), End Turn; бій запускається без помилок.
...
Глобальна карта            (8 год)   ✅  ← завершено (v4.8)

↓

UI polish (решта)          (4 год)   ✅  ← завершено (v4.9)
```
